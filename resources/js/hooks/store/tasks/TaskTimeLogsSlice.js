...
const createTaskTimeLogsSlice = (set, get) => ({
  ...
  deleteTimerLog: async (task, deleteId) => {
    ...
    try {
      await axios.delete(
        route('projects.tasks.time-logs.destroy', [task.project_id, task.id, deleteId]),
        { progress: true }
      );
  ...
  startTimer: async task => {
    ...
    try {
      const { data } = await axios.post(
        route('projects.tasks.time-logs.timer.start', [task.project_id, task.id]),
        {},
        { progress: true }
      );
  ...
  stopTimer: async (task, timeLogId) => {
    const taskIndex = get().tasks[task.group_id].findIndex(i => i.id === task.id);
    const index = get().tasks[task.group_id][taskIndex].time_logs.findIndex(
      i => i.id === timeLogId
    );

    try {
      const { data } = await axios.post(
        route('projects.tasks.time-logs.timer.stop', [task.project_id, task.id, timeLogId]),
        {},
        { progress: true }
      );
